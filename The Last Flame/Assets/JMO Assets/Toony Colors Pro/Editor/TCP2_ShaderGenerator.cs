...
public class TCP2_ShaderGenerator : EditorWindow
{
	...
	public class UIFeature_Mask : UIFeature
	{
		...
		public override void DrawGUI( TCP2_Config config )
		{
			GUIMask(config, this.label, this.tooltip, this.maskKeyword, this.channelKeyword, this.keyword, this.Enabled(config), this.increaseIndent, helpTopic: this.helpTopic);
		}
	}
	...
	public class UIFeature_ShaderTarget : UIFeature
	{
		public override void DrawGUI( TCP2_Config config )
		{
			...
			TCP2_GUI.SubHeader("Shader Target", "Defines the shader target level to compile for", config.shaderTarget != 30, 170 - 24f);
...
//		Space();
		...
		if(visible)
		{
			...
			else
			{
				...
				if(showHelp)
					...
					GUILayout.Space(24f);
				GUI.enabled = enabled;
				TCP2_GUI.SubHeader(label, tooltip, feature && enabled, (!helpIndent && !showHelp) ? 165 : 145);
				feature = EditorGUILayout.Toggle(feature);
			}
			EditorGUILayout.EndHorizontal();
		}
	...
	//static private bool GUIMultipleFeatures(TCP2_Config config, string label, string tooltip, params string[] labelsAndFeatures)
	//{
	//	return GUIMultipleFeaturesInternal(config, label, tooltip, labelsAndFeatures);
	//}
	...
	static private bool GUIMultipleFeaturesInternal(TCP2_Config config, string label, string tooltip,  string[] labelsAndFeatures, bool enabled = true, bool increaseIndentLevel = false, string helpTopic = null, bool showHelp = true, bool helpIndent = true, bool visible = true)
	{
		...
		visible = sHideDisabled ? enabled : visible;
		...
		if(visible)
		{
			...
			else if (helpIndent && !increaseIndentLevel)
				GUILayout.Space(24f);
			float width = (showHelp || (helpIndent && !increaseIndentLevel)) ? 146f : 166f;
	...
	static private bool GUIMask(TCP2_Config config, string label, string tooltip, string maskKeyword, string channelKeyword, string feature = null, bool enabled = true, bool increaseIndentLevel = false, bool visible = true, string helpTopic = null)
	{
